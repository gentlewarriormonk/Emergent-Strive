    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Generate invite codes for students to join your class
          </CardDescription>
        </CardHeader>
      ...
      {context.role !== 'student' && context.class_id && (
...
const Dashboard = () => {
  ...
  const setupRealTimeUpdates = () => {
    ...
    // Set up Supabase real-time subscriptions for habit_logs
    const channel = supabase
      ...
      .on('postgres_changes', 
        ...
        (payload) => {
          ...
          // Refresh habits when habit logs change
          fetchHabits();
        }
      )
      .subscribe();
  ...
  const toggleHabit = async (habitId, completed) => {
    try {
      ...
      // Show streak broken notification if needed
      if (completed && !completed) {
        // Stub for streak broken notification
  ...
  const tabs = [
    { id: 'habits', label: 'My Habits', icon: <Target className="h-4 w-4" /> },
    ...(context.role !== 'student' ? [
      { id: 'admin', label: 'Manage', icon: <Settings className="h-4 w-4" /> }
    ] : [])
  ];
